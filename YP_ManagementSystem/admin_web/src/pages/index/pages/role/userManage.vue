<template>
  <div class="border pd10">
    <el-row>
      <el-col v-if="show['用户列表']" class="pd10">
        <el-form :inline="true" :model="form" ref="form" class="demo-form-inline" label-width="70px">
          ...
          <el-form-item label="所属组织" size="mini" prop="orgid">
          ...
          <el-form-item label="入职状态" size="mini" prop="status">
            <el-select v-model="form.status" placeholder="请选择">
              <el-option :key="sta.ID" v-for="sta in statusList" :label="sta.Name" :value="sta.ID"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="账号类型" size="mini" prop="ismanage">
            <el-select v-model="form.ismanage" placeholder="请选择">
              <el-option :key="manage.ID" v-for="manage in manageList" :label="manage.Name" :value="manage.ID"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" :loading="loadingBtnState.searchState" @click="getUserList">查询</el-button>
    ...
    <el-row>
      <el-col :span="24" class="pd10">
        <el-button v-if="show['用户页添加用户']" type="success" icon="el-icon-plus" @click="addBefore">添加</el-button>
        <el-button v-if="show['用户页变更用户状态']" type="warning" icon="el-icon-edit" @click="changeStatusBefore" :disabled="!options.multipleSelection.length">状态变更</el-button>
        <el-button v-if="show['用户页变更用户组织']" type="primary" icon="el-icon-edit" @click="changeOrgBefore" :disabled="!options.multipleSelection.length">组织变更</el-button>
        <el-button v-if="show['用户页删除用户']" type="danger" icon="el-icon-delete" @click="removeUser" :disabled="!options.multipleSelection.length">删除</el-button>
      </el-col>
    </el-row>
    <el-row v-loading="userListLoading">
      <el-col :span="24" class="pd10">
    ...
    <el-dialog v-if="show['用户页添加用户']" title="添加用户" :visible.sync="addFormVisible" :before-close="confirmClose" @close="resetForm('addform')">
      <el-form :model="addform" ref="addform" :rules="rules" label-width="80px">
        <el-form-item label="所属组织" prop="orgid">
          <el-select v-model="addform.orgid" @change="getRoleByOrgID" placeholder="请选择所属组织" style="width:100%;">
            <el-option :key="org.ID" v-for="org in orgList" :label="org.Name" :value="org.ID"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="入职状态" prop="status">
          <el-select v-model="addform.status" placeholder="请选择入职状态" style="width:100%;">
            <el-option :key="status.ID" v-for="status in statusList" :label="status.Name" :value="status.ID"></el-option>
          </el-select>
        </el-form-item>
    ...
    <el-dialog v-if="show['用户页编辑用户']" title="编辑用户" :visible.sync="editFormVisible" @close="resetForm('editform')">
    ...
    <el-dialog v-if="show['用户页变更用户状态']" title="状态变更" :visible.sync="statusFormVisible" @close="resetForm('statusform')">
    ...
    <el-dialog v-if="show['用户页变更用户组织']" title="组织变更" :visible.sync="orgFormVisible" @close="resetForm('orgform')">
    ...
    <el-dialog v-if="show['用户页设置角色']" title="设置角色" :visible.sync="roleFormVisible" @close="resetForm('roleform')">
...
export default {
        ...
        name: [
          { required: true, message: "请输入姓名", trigger: "blur" },
          {
            validator: (rule, value, callback) => {
              if (!/(^[a-zA-Z\u4e00-\u9fa5]{1,19}$)/.test(value)) {
                callback(new Error("请输入2到20字符,请使用中文或大小写字母!"));
              } else {
                callback();
              }
            },
  ...
  methods: {
    init() {
      this.activeOrg = this.storage.get("userOrg");
      this.getOrgList();
      this.getRoleList();
    ...
    getRoleByOrgID() {
      ...
      this.ajax("用户页默认角色", { id: this.addform.orgid }, (ret, err) => {
        if (ret === null) {
          this.$message({
            message: err.msg,
            type: "error"
    ...
    getUserList() {
      this.loadingBtnState.searchState = true;
      let start = (this.pageindex - 1) * this.pagesize;
      let obj = { start: start, size: this.pagesize };
      for (let key in this.form) {
        if (this.form[key] !== "") obj[key] = this.form[key];
      }
      if (!obj.orgid) obj.orgid = this.activeOrg.id;
      this.userListLoading = true;
      this.ajax("用户列表", obj, (ret, err) => {
        this.userListLoading = false;
        this.loadingBtnState.searchState = false;
        if (ret === null) {
          this.$message({
            message: err.msg,
            type: "error"
    ...
    addUser() {
      this.$refs["addform"].validate(valid => {
        if (valid) {
          ...
          this.ajax("用户页添加用户", this.addform, (ret, err) => {
    ...
    changeStatus() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.ajax("用户页变更用户状态", this.statusform, (ret, err) => {
    ...
    changeRole() {
      ...
      this.ajax("用户页设置角色", this.roleform, (ret, err) => {
    ...
    changeOrg() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.ajax("用户页变更用户组织", this.orgform, (ret, err) => {
    ...
    editUserHandle() {
      this.$refs["editform"].validate(valid => {
        if (valid) {
          ...
          this.ajax("用户页编辑用户", this.editform, (ret, err) => {
            ...
            if (ret === null) {
              ...
              this.userList[this.activeIndex].OrgName = orgname;
              this.resetForm("editform");
              this.activeIndex = "";
              this.$message({
                type: "success",
    ...
    removeUser(row, index) {
      ...
      this.$confirm("确定要删除用户吗?", "提示", {
        ...
        .then(() => {
          this.ajax("用户页删除用户", data, (ret, err) => {
            if (ret === null) {
              this.$message({
                message: err.msg,
                type: "error"
    ...
    handleSelectionChange(val) {
      ...
      for (let i in val) {
        ...
        for (let j in this.userList) {
          if (this.userList[j].ID === val[i].ID) {
            indexArr.push(j);
          }
        }
      }
